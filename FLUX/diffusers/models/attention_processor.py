...
def position_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, ip_adapter_h=None, ip_adapter_w=None) -> torch.Tensor:
    ...
    if attn_mask is not None:
        if attn_mask.dtype == torch.bool:
            ...
            attn_bias += attn_mask
    attn_weight = query @ key.transpose(-2, -1) * scale_factor  # (b, num_head, l_q, l_k)

    attn_weight += attn_bias
    attn_weight = torch.softmax(attn_weight, dim=-1)


    position_attn_weight = attn_weight.chunk(4, dim=-1)

    # cofficient
    coefficients_h = np.linspace(1, 0, ip_adapter_h)
    inverse_coefficients_h = np.linspace(0, 1, ip_adapter_h)
    coefficients_w = np.linspace(1, 0, ip_adapter_w)
    inverse_coefficients_w = np.linspace(0, 1, ip_adapter_w)

    left_matrix = np.zeros((ip_adapter_h, ip_adapter_w))
    right_matrix = np.zeros((ip_adapter_h, ip_adapter_w))
    top_matrix = np.zeros((ip_adapter_h, ip_adapter_w))
    down_matrix = np.zeros((ip_adapter_h, ip_adapter_w))
    for i in range(ip_adapter_h):
        top_matrix[i, :] = coefficients_h[i]
        down_matrix[i, :] = inverse_coefficients_h[i]

    for i in range(ip_adapter_w):
        left_matrix[:, i] = coefficients_w[i]
        right_matrix[:, i] = inverse_coefficients_w[i]


    left_matrix = torch.from_numpy(left_matrix).flatten().unsqueeze(0).unsqueeze(0).unsqueeze(-1).detach().cuda().to(torch.float16) # (1, 1, L, 1)
    right_matrix = torch.from_numpy(right_matrix).flatten().unsqueeze(0).unsqueeze(0).unsqueeze(-1).detach().cuda().to(torch.float16)
    top_matrix = torch.from_numpy(top_matrix).flatten().unsqueeze(0).unsqueeze(0).unsqueeze(-1).detach().cuda().to(torch.float16)
    down_matrix = torch.from_numpy(down_matrix).flatten().unsqueeze(0).unsqueeze(0).unsqueeze(-1).detach().cuda().to(torch.float16)


    attn_weight_left = position_attn_weight[0] * left_matrix
    attn_weight_right = position_attn_weight[1] * right_matrix
    attn_weight_top = position_attn_weight[2] * top_matrix
    attn_weight_down = position_attn_weight[3] * down_matrix

    attn_weight = torch.cat([attn_weight_left, attn_weight_right, attn_weight_top, attn_weight_down], dim=-1)


    
    attn_weight = torch.dropout(attn_weight, dropout_p, train=True).to(torch.bfloat16)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        quiet_attn_parameters = {"ip_adapter_masks", "ip_hidden_states", "ip_adapter_h", "ip_adapter_w"}
        unused_kwargs = [
...
class FluxIPAdapterJointAttnProcessor2_0(torch.nn.Module):
    ...
    def __init__(
        self, hidden_size: int, cross_attention_dim: int, num_tokens=(4,), scale=0.2, device=None, dtype=None
    ):
        super().__init__()
    ...
    def _init_layers(self):
        """ 对 ModuleList 中的每个线性层进行初始化 """
        for layer_list in [self.to_k_ip, self.to_v_ip]:
            for linear_layer in layer_list:
                # 权重初始化（关键）
                nn.init.kaiming_normal_(
                    linear_layer.weight, 
                    mode='fan_in',        # 适用于后续接激活函数的场景
                    nonlinearity='leaky_relu'  # 根据实际激活函数调整
                )
                
                # 偏置初始化（建议保持较小值）
                if linear_layer.bias is not None:
                    nn.init.normal_(linear_layer.bias, mean=0, std=0.01)
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None:
            ...
            for current_ip_hidden_states, scale, to_k_ip, to_v_ip in zip(
                ...
                # current_ip_hidden_states : (B, N_q, C)
                # ip_query: [B, Num_head(24), L, head_dim(128)]
                ...
                # New
                current_ip_hidden_states = position_scaled_dot_product_attention(
                    ip_query, ip_key, ip_value, attn_mask=None, dropout_p=0.0, is_causal=False, ip_adapter_h=ip_adapter_h, ip_adapter_w=ip_adapter_w
                )
                ...
                # ip_attn_output += scale * current_ip_hidden_states * ip_adapter_masks.to(ip_query.dtype)

            # New
            # hidden_states += ip_attn_output
            hidden_states += scale * current_ip_hidden_states * ip_adapter_masks.to(ip_query.dtype)
